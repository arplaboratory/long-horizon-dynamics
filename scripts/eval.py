...
if __name__ == "__main__":
    ...
    Y_plot = np.zeros((X.shape[0],     6))
    Y_hat_plot = np.zeros((X.shape[0], 6))
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(0, X.shape[0])):
            ...
            linear_velocity_gt =  y[:3].detach().cpu().numpy()
            angular_velocity_gt = y[7:10].detach().cpu().numpy()

            velocity_gt = np.hstack((linear_velocity_gt, angular_velocity_gt))

            Y_plot[i, :] = velocity_gt
